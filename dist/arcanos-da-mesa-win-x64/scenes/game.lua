...
local function load_ui_font()
    ...
    if aegis.loadDefaultFont then
        for _, size in ipairs({12, 13, 14, 15, 16, 18, 20, 22, 24}) do
...
local function pick_font(size)
    return fonts[size] or fonts[16] or fonts[18] or fonts[14]
end

local function make_label(text, size)
    if aegis.newLabelSize then
        local ok, l = pcall(aegis.newLabelSize, text, size or 16)
...
local function label(text, x, y, r, g, b, z, size)
    local l = make_label(text, size or 16)
    local font = pick_font(size or 16)
...
local function button(text, x, y, w, h)
    ...
    b.label = label(text, x + 18, y + 12, 0.94, 0.88, 0.70, 32, 16)
    return b
end
...
local function energy_panel()
    ...
    local title = label("ENERGIA", 452, 50, 0.98, 0.82, 0.34, 25, 20)
    ...
    local value = label(energy .. " / " .. max_energy, 790, 53, 0.80, 0.92, 1.00, 26, 20)
end
...
local function draw_card(card, x, y, playable, slot)
    ...
    label(d.name, x + 18, y + 17, 0.06, 0.04, 0.06, 16, 14)
    label("Tipo: " .. d.kind, x + 18, y + 54, 0.95, 0.86, 0.62, 16, 14)
    rect(x + 20, y + 80, CARD_W - 40, 42, d.r * 0.58, d.g * 0.58, d.b * 0.58, 14)
    label("Custo: " .. d.cost .. "     Vida da carta: " .. card.life .. "/3", x + 18, y + 132, 0.92, 0.88, 0.72, 16, 13)
    label("Ataque " .. d.atk .. " | Defesa " .. d.block .. " | Cura " .. d.heal, x + 18, y + 156, 0.84, 0.90, 0.92, 16, 13)
    label("Efeito: " .. d.text, x + 18, y + 182, 0.66, 0.72, 0.75, 16, 12)
    if playable then
        rect(x, y, 6, CARD_H, 0.96, 0.72, 0.20, 17)
        label("CLIQUE PARA USAR", x + 30, y + 202, 0.98, 0.86, 0.40, 18, 12)
    else
...
local function render()
    ...
    local turn_lbl = label("Turno " .. turn .. " - " .. (phase == "player" and "sua vez" or "vez da IA"), 92, 126, 0.96, 0.78, 0.32, 20, 16)
    energy_panel()

    rect(166, 190, 92, 132, 0.20, 0.52, 0.36, 4)
    rect(186, 162, 52, 52, 0.70, 0.86, 0.66, 5)
    label("Voce", 176, 338, 0.72, 0.90, 0.78, 8, 14)
    rect(1018, 190, 92, 132, 0.52, 0.16, 0.20, 4)
    rect(1038, 162, 52, 52, 0.86, 0.54, 0.48, 5)
    label("IA", 1052, 338, 0.95, 0.65, 0.60, 8, 14)

    for i = 1, 3 do card_back(458 + (i - 1) * 126, 136) end

    rect(260, 374, 760, 68, 0.055, 0.050, 0.075, 18)
    label(message, 286, 386, 0.86, 0.88, 0.78, 20, 14)
    label("Regras: ataque gasta 2 de vida da carta; defesa/cura gasta 1; vida 0 vai ao cemiterio.", 262, 420, 0.58, 0.68, 0.70, 20, 13)
    ...
    label("Seu deck " .. #player_deck .. " | Seu cemiterio " .. #player_grave, 84, 684, 0.56, 0.66, 0.70, 20, 13)
    label("Deck IA " .. #enemy_deck .. " | Cemiterio IA " .. #enemy_grave, 880, 684, 0.56, 0.66, 0.70, 20, 13)
