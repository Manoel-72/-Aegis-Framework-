...
-- ── Estado da cena ──────────────────────────────────────────────────
local player, playerCol
local bullets       = {}   -- { obj, c, vx, vy, life }
local enemies       = {}   -- { obj, c, hp, pathTimer, path, pathIndex, floatY }
...
local inv_labels    = {}
local mission_label, mission_obj
    ...
    aegis.onCollideEnter(c, function(a, b)
        ...
        for i = #bullets, 1, -1 do
            if bullets[i].c == b then
                ...
                -- dano ao inimigo
                e.hp = e.hp - 1
                aegis.flashScreen({ r=1, g=0.3, b=0.3 }, 0.04)
                ...
                if e.hp <= 0 then
                    ...
                    aegis.playSound("audio/enemy_die.wav")
                    ...
                    if m and m.check() and not missionDone then
                        ...
                        aegis.playSound("audio/mission_complete.wav")
                        aegis.screenShake(4, 0.3)
                    end
                else
                    aegis.playSound("audio/hit.wav")
...
local function spawn_wave()
    ...
    aegis.playSound("audio/wave_start.wav")
end
...
local function spawn_item(kind, x, y)
    ...
    aegis.onCollideEnter(c, function(a, b)
        ...
        aegis.playSound("audio/collect.wav")
        aegis.burst(aegis.getX(obj), aegis.getY(obj), {
...
local function shoot()
    if GAME.inventory.ammobox <= 0 and GAME.zone > 1 then
        ...
        aegis.playSound("audio/empty.wav")
        return
    end
    ...
    aegis.playSound("audio/shoot.wav")
...
local function build_hud()
    ...
    -- Zona atual
    local zone_lbl = aegis.newLabel("ZONA " .. GAME.zone .. " / " .. GAME.maxZone)
    aegis.setPosition(zone_lbl, sw / 2, 14)
    ...
    aegis.setPosition(mission_label, sw / 2, 36)
    aegis.setColor(mission_label, 0.8, 0.9, 0.6)
    aegis.setZ(mission_label, 500)

    local mission_desc2 = aegis.newLabel(m and m.desc2 or "")
    aegis.setPosition(mission_desc2, sw / 2, 56)
...
function aegis_init()
    ...
    aegis.playMusic("audio/game.ogg")
...
function aegis_update(dt)
    ...
    -- Move player com limites de mundo
    local nx = math.max(P_SIZE, math.min(WORLD_W - P_SIZE, aegis.getX(player) + vx * dt))
    local ny = math.max(P_SIZE, math.min(WORLD_H - P_SIZE, aegis.getY(player) + vy * dt))
    aegis.setPosition(player, nx, ny)
    ...
    for _, e in ipairs(enemies) do
        ...
        if math.sqrt(epx*epx + epy*epy) < 32 and hurtCooldown <= 0 then
            ...
            aegis.playSoundAt("audio/hurt.wav",
