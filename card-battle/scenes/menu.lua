...
local function button(text, x, y, action, enabled)
    ...
    b.label = label(text, x + 32, y + 16, 0.92, 0.88, 0.76, 8)
    aegis.setScale(b.label, 1.12, 1.12)
    buttons[#buttons + 1] = b
    return b
end
...
local function toggle_extra()
    ...
    if extra_open then
        ...
        extra_nodes[#extra_nodes + 1] = label("Extra", 482, 450, 0.96, 0.78, 0.30, 32)
        extra_nodes[#extra_nodes + 1] = label("Jogo bom demais: cartas com texto, custo, vida e uma IA sem vergonha.", 482, 492, 0.78, 0.86, 0.86, 32)
        extra_nodes[#extra_nodes + 1] = label("Este prototipo e separado do Zone Recon.", 482, 524, 0.60, 0.68, 0.72, 32)
    end
end
...
function aegis_init()
    ...
    title_node = label("ARCANOS DA MESA", 80, 78, 0.96, 0.80, 0.38, 10)
    aegis.setScale(title_node, 1.65, 1.65)
    if aegis.tween then
        aegis.tween(title_node, { scaleX = 1.72, scaleY = 1.72 }, 0.9, "inout", nil, { loop=true, yoyo=true })
    end
    label("Duelo tatico de cartas: leia, escolha, ataque, defenda e sobreviva.", 84, 146, 0.75, 0.84, 0.86, 10)
    label("Cada carta tem nome, custo, vida, ataque, defesa, cura e efeito escrito.", 84, 178, 0.63, 0.73, 0.78, 10)
    ...
    label("Novo jogo: comeca um duelo do zero.", 420, 278, 0.74, 0.82, 0.84, 10)
    label("Continuar: volta ao HP salvo do prototipo.", 420, 348, 0.74, 0.82, 0.84, 10)
    label("Extra: mostra uma nota divertida.", 420, 418, 0.74, 0.82, 0.84, 10)
    label("Exit: fecha o jogo.", 420, 488, 0.74, 0.82, 0.84, 10)
    label("Mouse: clique nos botoes. Teclado: W/S + Enter.", 98, 588, 0.58, 0.68, 0.72, 10)
end
...
function aegis_update(dt)
    ...
    for i, b in ipairs(buttons) do
        ...
        elseif i == selected then
            ...
            aegis.setScale(b.label, 1.20, 1.20)
        else
